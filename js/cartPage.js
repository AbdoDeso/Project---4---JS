...
function addItems () {
    ...
    return `<div  id="${item.id}" price="${item.price}" class=" hover:shadow-2xl hover:-translate-y-4 h-112 w-80  xl:w-80 bg-gray-200 rounded-sm
     border-1 border-gray-300  transition duration-300">
            ...
            <div class="flex flex-col px-5 mx-auto py-5  text-left  gap-y-2 ">
                    ...
                    <p class="inline text-sm">Price : ${item.price}.LE</p>
                      <div class="mt-[-65px] mb-[20px] ml-45 space-x-1  ">
                        <button class="border border-1 px-[5px] md:px-[10px] text-xs md:text-2xl"  onClick="itemCounter(${item.id}, -1)">-</button>
                        <span class="w-5 text-sm md:text-2xl" id="counter-${item.id}">1</span>
                        <button class="border border-1 px-[5px] md:px-[10px] text-xs md:text-2xl" onClick="itemCounter(${item.id}, +1)">+</button>                                       
                    </div>
            </div>
                <div class="flex items-center justify-center">
                         <button class=" rounded-lg  md:py-1 px-2 md:px-4  font-bold bg-red-700 text-white hover:bg-red-800 hover:text-white
...
function removeProduct (id) {
    ...
    let countPrices = localStorage.getItem("price")

    let minPrice = countPrices - (findItem.price * getItemValue)
